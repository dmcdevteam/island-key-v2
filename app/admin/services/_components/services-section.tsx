...
const IN_HOUSE_SUBS     = ['wellness_health', 'family_care', 'food_dining', 'villa_lifestyle', 'private_experiences']
const RESERVATIONS_SUBS = ['beach_dining_nightlife', 'lifestyle_shopping', 'events_access']
...
function ServiceForm({ service, onSave, onClose }: ServiceFormProps) {
  ...
  const subcatOptions = form.category === 'in_house' ? IN_HOUSE_SUBS : RESERVATIONS_SUBS
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                value={form.category}
                onChange={e => setForm(f => ({ ...f, category: e.target.value as Service['category'], subcategory: e.target.value === 'in_house' ? 'wellness_health' : 'beach_dining_nightlife' }))}
                className={SELECT}
              >
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-border-light text-left">
                {['Title', 'Category', 'Subcategory', 'Service Type', 'Price Label', 'Active', 'Featured', 'Actions'].map(h => (
            ...
            <tbody>
              {services.map(s => (
                <tr key={s.id} className="border-b border-border-light hover:bg-sand/30 transition-colors">
                  ...
                  <td className="py-2.5 pr-4 text-tx-mid">{SUBCATEGORY_LABELS[s.subcategory] ?? s.subcategory}</td>
...
// ─── Tab 2: Subcategory Images ────────────────────────────────────────────────
const ALL_SUBCATEGORIES = [
  ...IN_HOUSE_SUBS.map(k => ({ key: k, label: SUBCATEGORY_LABELS[k], category: 'In-House' as const })),
  ...RESERVATIONS_SUBS.map(k => ({ key: k, label: SUBCATEGORY_LABELS[k], category: 'Reservations' as const })),
]
...
function SubcategoryImagesTab() {
  ...
  useEffect(() => {
    fetch('/api/services/subcategories')
      .then(r => r.json())
      .then(d => setSubcats(Array.isArray(d.subcategories) ? d.subcategories : []))
      .catch(() => {})
  }, [])
  ...
  async function handleUpload(subcatKey: string, file: File) {
    ...
    try {
      ...
      showToast(`Updated image for ${SUBCATEGORY_LABELS[subcatKey]}`)
    } catch (e) {
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {ALL_SUBCATEGORIES.map(({ key, label, category }) => {
          const meta       = subcats.find(s => s.subcategory === key)
          const currentImg = meta?.image_wide ?? meta?.image_url ?? null
          const busy       = uploading === key

          return (
            <div key={key} className="border border-border-light rounded-sm overflow-hidden bg-white">
              {/* Image preview */}
              ...
              <div className="p-2.5">
                <p className="text-[11px] font-bold text-navy leading-tight">{label}</p>
                <p className="text-[10px] text-tx-light mb-2">{category}</p>
                <input
                      ...
                      <tr key={`${e.id}-detail`} className="border-b border-border-light bg-sand/20">
                        <td colSpan={6} className="px-4 py-3">
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                            ...
                            {notes.subcategory && <div><span className="text-[10px] font-bold text-tx-mid uppercase">Subcategory</span><p className="text-navy">{SUBCATEGORY_LABELS[notes.subcategory] ?? notes.subcategory}</p></div>}
...
// ─── Main Component ───────────────────────────────────────────────────────────
const TABS = ['Services', 'Subcategory Images', 'Service Enquiries'] as const
